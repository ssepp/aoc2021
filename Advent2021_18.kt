...
const val test_input = """[[[[[9,8],1],2],3],4]"""
...
val snails = mutableListOf<Value>()

data class Value(var pair: MutablePair<Value, Value>?, var num:Int?) {
...
fun parseInput() {
    ...
    for (line in lines) {
        ...
        val valueStack = mutableListOf<Value>()
        val writeLeftStack = mutableListOf<Boolean>()

        var currentValue = Value(null,null)
        snails.add(currentValue) // add the outermost value

        valueStack.add(currentValue)
        writeLeftStack.add(true)

        nextSnail@for (c in line.substring(1)) {

            if (c == '[') {
                val oldValue = currentValue
                currentValue = Value(null,null)

                if (writeLeftStack.last()) {
                    oldValue.pair = MutablePair<Value,Value>()
                    oldValue.pair!!.left = currentValue
                } else {
                    oldValue.pair!!.right = currentValue
                }
                ...
                if (valueStack.isEmpty()) {
                    ...
                    currentValue = valueStack.removeLast()
                    writeLeftStack.removeLast()
                }
            } else if (c == ',') {
                writeLeftStack.removeLast()
                writeLeftStack.add(false)
            } else {
                val num = c.toString().toInt()

                if (currentValue.pair == null) {
                    currentValue.pair = MutablePair()
                }

                if (writeLeftStack.last()) {
                    currentValue.pair!!.left = Value(null, num)
                } else {
                    currentValue.pair!!.right = Value(null, num)
                }
            }
